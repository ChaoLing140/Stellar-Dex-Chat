...
import { X, Loader2, CheckCircle, AlertCircle, ArrowDownUp } from 'lucide-react';
import { useStellarWallet } from '@/contexts/StellarWalletContext';
import { depositToContract, withdrawFromContract, stroopsToDisplay } from '@/lib/stellarContract';
    ...
    const handleAction = async () => {
        ...
        try {
            ...
            if (isAdminMode) {
                ...
                hash = await withdrawFromContract(connection.publicKey, to, stroopsAmount, signTx);
            } else {
                hash = await depositToContract(connection.publicKey, stroopsAmount, signTx);
            }
            setTxHash(hash);
            setStatus('success');
        } catch (err) {
                ...
                {status === 'success' ? (
                    ...
                    <>
                        ...
                        <div className="flex justify-between text-xs text-gray-500 mb-6">
                            <span>Connected: {connection.address.slice(0, 8)}…{connection.address.slice(-4)}</span>
